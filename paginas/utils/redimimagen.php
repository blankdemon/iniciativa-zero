...
class Imagen {
	...
	function Imagen($url=null, $width=null, $height=null) {
		$url = trim($url);
		if(empty($url)) {
		...
		if($width) $this->wDefect = $width;
		if($height) $this->hDefect = $height;
	...
	function resize($maxancho=null,$maxalto=null,$cut=false) {
		if($this->stop()) 
			return;
	...
	function error($msg) {
		...
		$this->rw = $this->wDefect;
		$this->rh = $this->hDefect;
		...
		imagestring ($this->source, 5, 35, 20, "Image", $text_color_red);
		imagestring ($this->source, 5, 19, 50, "not found.", $text_color_red);
	...
	function doSave($destination,$quality=null) {
		$quality = (is_numeric($quality) and $quality <= 100 and $quality >= 1) ? intval($quality) : 75;
		Header("Content-type: image/".$this->type);
		$f2 = $this->f2;
	...
	function doDownload($quality=null) {
		$quality = (is_numeric($quality) and $quality <= 100 and $quality >= 1) ? intval($quality) : 75; 
	...
	function watermark($file,$posx = 0,$posy = 0,$loop=false,$rotation = 0) {
		if($this->stop()) 
			return;
	...
	function imagecopy2(&$s, $wm, $posx, $posy, $srcx, $srcy, $wm_w, $wm_h, $pct = null) {
		if($this->type == "gif")
	...
	function textmark($text,$color,$size,$font=null,$rot=0,$posx = 0,$posy = 0) {
		if($this->stop()) 
			return;
	...
	function colorize($color,$exact=false) {
		if($this->stop()) 
			return;
		if($color = $this->rgbhex2rgb($color)) {
			if($exact == true)
